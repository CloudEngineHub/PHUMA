...
def parse_args():
    parser = argparse.ArgumentParser(description="Preprocess G1 folder")
    ...
    parser.add_argument("--min_com_height_threshold", type=float, default=0.4)
    parser.add_argument("--com_to_bos_distance_threshold", type=float, default=0.16)
...
def main(args):
    humanoid_pose_folder = os.path.join(args.project_dir, 'data', 'preprocessed_original', 'g1')
    humanoid_model_dir = os.path.join(args.project_dir, "asset", "humanoid_model", 'g1')
            ...
            while start_frame < N:
                ...
                if is_valid:
                    ...
                    if args.output_dir is None:
                        output_dir = os.path.join(args.project_dir, 'data', 'g1')
                    else:
                    ...
                    chunk_dof_pos = np.concatenate([chunk_dof_pos[:, :19], np.zeros((N_chunk, 3)), chunk_dof_pos[:, 19:], np.zeros((N_chunk, 3))], axis=1)
                    chunk_motion_data = {
